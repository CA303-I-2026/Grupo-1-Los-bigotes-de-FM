...
\begin{figure}[H]
    ...
    \caption*{Conteos observados en transición entre tips de datos}
    \label{fig:miimagen}
\end{figure}
...
La prueba chi cuadrado de independecia sobre la matriz de transiciones rechaza la hipotesis nula de independencia entre el tipo origen y destino.
...
Una limitación es que se eligió arbitrariamente el método de tokenización, ya que es imposible con la capacidad computacional actual, decidir cual modelo es el más eficiente. 

Otra limitación es que el dataset brindado por RockYou.txt está limitado a un sector de la población en específico,lo que provocó que la idea inicial del proyecto de buscar predecir contraseñas no se pudiera llevar acabo, pues en general los resultados no se habrían podido generalizar al resto de la población. 
...
Prompt: Prompts iterativos para la generación de gráficos en Python de las nuevas funciones de análisis (chi2_independencia_transiciones y spearman_coocurrencia_tipos) y la corrección  y finalizacions de los algoritmos de las mismas funciones
